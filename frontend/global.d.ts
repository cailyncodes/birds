// From https://gist.github.com/ackvf/de21847e78083034252961d550963579#file-global-d-ts-L117
////// -----------------------------------------------------------------------------------------------------------------
/*//// -----------------------------------------------------------------------------------------------------------------
...
/** Takes a whole type and lets you modify some properties. */
type Modify<T, R extends PartialAny<T>> = Omit<T, keyof R> & R

/** Lets you pick properties from another type and modify them. This is useful to - 1. link two types together, - 2. provide intellisense, - 3. allow easier refactoring. */
type ModifyPick<T, R extends PartialAny<T>> = R
...
/** Makes properties deeply partial. Optionally specify override type for all leafs. */
type DeepPartialModify<T, RT = { __: false }> = {
  [P in keyof T]?: T[P] extends AnyObject ? DeepPartialModify<T[P], RT> : RT extends { __: false } ? T[P] : RT
}
...
/** Useful with `satisfies T` when additional properties need to be allowed. */
type DeeplyExtensible<T> = T extends AnyObject ? { [P in keyof T]: DeeplyExtensible<T[P]> } & AnyObject : T
