...
class BirdSpotManager:
    ...
    def get_missing_species(
        ...
        for s in possible_species:
            try:
                if isinstance(s, dict):
                    taxon_order = s.get("taxonOrder", 0)
                    if (
                        str(int(taxon_order)) not in seen_species
                        and int(taxon_order) != 0
                    ):
                        result.append(s)
                else:
                    continue
    ...
    async def get_scores_for_region(
        ...
        for hotspot in hotspots:
            ...
            for date in dates:
                ...
                species = (
                    await self.ebird_manager.get_species_observed_by_date_and_region(
                        loc_id, date, auth
                    )
                )
                checklists = await self.ebird_manager.get_checklists_by_date_and_region(
                    loc_id, date, auth
                )
            ...
            missing_species = dict(
                ...
                if key in missing_species_codes
            )
            hotspot_scores.append(
                {
                    "location": hotspot,
                    "missing_species": missing_species,
                    "birdspot_score": self.get_target_score_for_hotspot_and_date(
                        missing_species, checklists_map, target_date
                    ),
        ...
        return sorted(
            hotspot_scores,
            key=lambda hotspot_score: hotspot_score.get("birdspot_score"),
            reverse=True,
        )

    def get_target_score_for_hotspot_and_date(
        self,
        all_observations: dict,
        checklists_map: dict[datetime.date, list],
        ...
        for date, score_mult in zip(scoring_dates, score_mults):
            ...
            species_count = (
                len(species_seen_on_date[date])
                if len(checklists_map[date]) > 0
                else (
                    (
                        0.8
                        * (sum([len(s) for (_, s) in species_seen_on_date.items()]) + 1)
                        / (len(species_seen_on_date) + 1)
                    )
                    / ((target_date - date).days + 1)
                )
            )
